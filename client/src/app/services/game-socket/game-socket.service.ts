...
import { GamePageComponent } from '@app/pages/game-page/game-page.component'; // adjust the path as needed
...
export class GameSocketService {
    ...
    initializeSocketListeners(component: GamePageComponent): void {
        ...
        component.lobby = lobby ? (JSON.parse(lobby) as Lobby) : component.lobby; // lobby peut être inutile, car on a accesscode
        ...
        this.socketClientService.onAttackResult((data: { success: boolean; attackScore: number; defenseScore: number }) => {
            if (data.success) {
                console.log(`Combat réussi score attaque: ${data.attackScore}, score défense: ${data.defenseScore}`);
            } else {
                console.log(`Combat perdu score attaque: ${data.attackScore}, score défense: ${data.defenseScore}`);
            }
        });
